...
class Article extends AdminBase
{
    ...
    public function index($cid = 0, $keyword = '', $page = 1)
    {
        ...
        if ($cid > 0) {
            $category_children_ids = $this->category_model->where(new Where(['path' => ['like', "%,{$cid},%"]]))->column('id');
        ...
        $article_list  = $this->article_model->field($field)->where(new Where($map))->order(['publish_time' => 'DESC'])->paginate(15, false, ['page' => $page]);
